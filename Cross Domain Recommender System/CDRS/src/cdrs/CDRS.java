...
public class CDRS {
     ...
     // Searching user's genre in books database
  public int GenreSearch(List <String> temp, String userInput){
      ...
      return flag;
       ...
       //checking if user's book exists in database
       //if it does then storing the genre of the book
   ...
   ResultSet(UserGenre);
     ...
     //mapping book genre with movie genre
   ...
   //printing 3 random recommended movies between rating 3.5 and 4.8
       ...
       System.out.println("Movies Recommendations For you are:");
          while(count < 3){
              ...
              if(!unique.contains(i))
               if(Float.parseFloat(temp.get((3*(i+1)) -2)) <= 4.8 && Float.parseFloat(temp.get(3*(i+1) -2)) >= 2 && count < 3.5){
